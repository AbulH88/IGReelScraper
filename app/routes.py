...
def dashboard():
    ...
    reels = []
    if not clear_view:
        query = Reel.query.order_by(Reel.last_views.desc().nullslast(), Reel.created_at.desc())
    ...
    if not recent_searches and not clear_view:
        bootstrap_tags = sorted({reel.source_hashtag for reel in reels if reel.source_hashtag})
    ...
    grouped_tags = active_hashtags or [state.hashtag for state in recent_searches]
    if not grouped_tags:
        grouped_tags = sorted({reel.source_hashtag for reel in reels if reel.source_hashtag})
    grouped_reels = []
    if not clear_view:
        for hashtag in grouped_tags:
            ...
            if tag_reels or state:
                grouped_reels.append({'hashtag': hashtag, 'reels': tag_reels, 'state': state})
        if grouped_reels:
            available_hashtags = {group['hashtag'] for group in grouped_reels}
            if selected_hashtag not in available_hashtags:
                selected_hashtag = grouped_reels[0]['hashtag']
            grouped_reels = [group for group in grouped_reels if group['hashtag'] == selected_hashtag]
    ungrouped_reels = [] if clear_view else [reel for reel in reels if not reel.source_hashtag]
    stats = {
        'reel_count': len(reels),
        'tracked_hashtags': len({tag for reel in reels for tag in reel.hashtag_list}),
        'avg_views': int(sum((reel.last_views or 0) for reel in reels) / len(reels)) if reels else 0,
        'max_views': max((reel.last_views or 0) for reel in reels) if reels else 0,
        'playable_reels': sum(1 for reel in reels if reel.playable_url),
    }
...
def discover():
    ...
    imported, errors, search_state = import_discovered_reels(hashtags)
    for tag, state in search_state.items():
        ...
        record.page = state.get('page') or 1
        record.next_page = state.get('next_page')
...
def discover_more(hashtag: str):
    ...
    next_page = tag_state.next_page if tag_state else None
    if not next_page:
        flash(f'No more saved results are available for #{hashtag} right now.', 'warning')
        return redirect(url_for('main.dashboard'))

    imported, errors, search_state = import_discovered_reels([hashtag], page_by_tag={hashtag: next_page})
    if hashtag in search_state:
        state = search_state[hashtag]
        record = tag_state or HashtagSearchState(hashtag=hashtag)
        record.page = state.get('page') or next_page
        record.next_page = state.get('next_page')
        record.more_available = bool(state.get('more_available'))
        db.session.add(record)
        db.session.commit()
...
def stream_reel_video(reel_id: int):
    ...
    headers = _instagram_headers(get_instagram_session(), referer=reel.url)
    cookies = _instagram_cookies(get_instagram_session())
    ...
    clean_headers = {key: value for key, value in passthrough.items() if value}
    return Response(
